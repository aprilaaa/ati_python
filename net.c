...
int main(int * a,int * b,int * c ,int * d,int * e,int * f) {
	int socketHandle;			/* Handle to UDP socket used to communicate with Net F/T. 处理用于Net F/T通讯的UDP套接字*/
	struct sockaddr_in addr;	/* Address of Net F/T. Net F/T的ip地址*/
	struct hostent* he;			/* Host entry for Net F/T. Net F/T的主机条目*/
	byte request[8];			/* The request data sent to the Net F/T. 发送到Net F/T的请求数据*/
	RESPONSE resp;				/* The structured response received from the Net F/T. 从Net F/T接受到的结构体*/
	byte response[36];			/* The raw response data received from the Net F/T. 从Net F/T接受到的原始数据*/
